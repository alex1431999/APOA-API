...
class Test(BaseTest):
    ...
    def test_plotting_data_no_data(self):
        ...
        self.assertEqual(
            response_data,
            [],
            "Since there is no plotting data, an empty list should be returned",
        )
    ...
    def test_plotting_data_with_data(self):
        ...
        tweet = self.mongo_controller.add_crawl_twitter(
            keyword_id, 123, "some text", 15, 15, timestamp, return_object=True
        )
        tweet = self.mongo_controller.set_score_crawl(
            tweet["_id"], score, return_object=True
        )

        response = self.send_request(keyword_id, self.user_1)
        response_data = response.get_json()

        plotting_data_expected = [
            {"score": score, "text": text, "timestamp": timestamp}
        ]
